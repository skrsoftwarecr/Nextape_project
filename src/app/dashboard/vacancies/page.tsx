...
import { PlusCircle, Loader2, Users, Calendar, ArrowRight, MoreVertical } from "lucide-react";
    ...
    <div className="space-y-12">
      ...
      <div className="grid grid-cols-1 gap-6">
        {vacancies.length > 0 ? vacancies.map((job) => (
          <div key={job.id} className="bg-white rounded-[2.5rem] p-8 md:p-10 border border-gray-50 shadow-apple flex flex-col md:row justify-between items-center gap-8 group hover:shadow-apple-lg transition-all">
            <div className="flex-grow space-y-6 w-full">
               <div className="flex justify-between items-start">
                  <div className="space-y-2">
                     <h2 className="text-2xl font-bold italic leading-none">{job.title}</h2>
                     <div className="flex items-center gap-4 text-[10px] font-bold uppercase tracking-widest text-gray-400">
                        <span className="flex items-center gap-1.5"><Calendar className="h-3 w-3" /> {format(job.postedAt.toDate(), "d MMM, yyyy", { locale: es })}</span>
